  ...
  try {
    ...
    const ponto = await prisma.rastreamento.create({
      data: {
        ...
        ignicao,
        hodometro:  hodometro  ? parseFloat(hodometro)  : null,
      },
    });
    if (hodometro) {
      await prisma.veiculo.update({
        where: { id: Number(veiculoId) },
        data: { kmAtual: parseFloat(hodometro) },
      });
    }
...
export default { posicaoAtual, historicoPorVeiculo, registrarPosicao, sincronizarGPS };
