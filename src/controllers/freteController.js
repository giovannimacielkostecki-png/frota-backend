...
async function calcular(req, res, next) {
  try {
    ...
    const resultado = calcularValores({ distanciaKm, precoDiesel, consumoKmL, pedagio, diariaMot, margemLucro });
    res.json({
      ...resultado,
      distanciaKm,
...
async function salvar(req, res, next) {
  try {
    ...
    const vals = calcularValores({ ...dados, precoDiesel, consumoKmL });
